...
from .config_xlerobot_single_arm import XLerobotSingleArmConfig, XLerobotSingleArmClientConfig


class XLerobotSingleArmClient(Robot):
    config_class = XLerobotSingleArmClientConfig
    name = "xlerobot_single_arm_client"

    def __init__(self, config: XLerobotSingleArmClientConfig):
        ...
        # For compatibility with record.py
    ...
    def connect(self) -> None:
        ...
        if self._is_connected:
            raise DeviceAlreadyConnectedError(
                "XLerobot Single Arm Client is already connected. Do not run `robot.connect()` twice."
            )
        ...
        if self.zmq_observation_socket not in socks or socks[self.zmq_observation_socket] != zmq.POLLIN:
            raise DeviceNotConnectedError("Timeout waiting for XLerobot Single Arm Host to connect expired.")
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError("XLerobot Single Arm Client is not connected. You need to run `robot.connect()`.")
    ...
    def send_action(self, action: dict[str, Any]) -> dict[str, Any]:
        """Command robot to move to a target joint configuration. Translates to motor space + sends over ZMQ
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "XLerobot Single Arm Client is not connected. You need to run `robot.connect()`."
            )
    ...
    def disconnect(self):
        ...
        if not self._is_connected:
            raise DeviceNotConnectedError(
                "XLerobot Single Arm Client is not connected. You need to run `robot.connect()` before disconnecting."
            )
